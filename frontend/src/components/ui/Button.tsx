...
const VARIANT: Record<Variant, string> = {
  primary: 'bg-accent text-on-accent border-accent hover:opacity-95',
  secondary: 'bg-paper text-ink border-rule hover:bg-pill',
  ghost: 'bg-transparent text-accent border-transparent hover:bg-tint',
  danger: 'bg-transparent text-danger border-rule hover:bg-danger-bg',
};
 ...
 * Focus comes from the global :focus-visible outline (globals.css).
 */
  ...
  return (
    <>
      <button
        ...
        className={`inline-flex items-center justify-center gap-2 rounded-control border font-semibold whitespace-nowrap transition-colors ${SIZE[size]} ${VARIANT[variant]} ${disabled ? 'opacity-50 cursor-not-allowed' : ''} ${className}`}
        disabled={disabled}
