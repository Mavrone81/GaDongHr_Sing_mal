...
const STAGE_COLORS: Record<string, string> = {
  ...
  INTERVIEW_1: 'bg-indigo-50 text-indigo-700 border-indigo-100',
  INTERVIEW_2: 'bg-violet-50 text-violet-700 border-violet-100',
...
const STAGE_DOT: Record<string, string> = {
  APPLIED: 'bg-slate-400', SCREENING: 'bg-blue-500', INTERVIEW_1: 'bg-indigo-500',
  INTERVIEW_2: 'bg-violet-500', ASSESSMENT: 'bg-amber-500', OFFER: 'bg-orange-500',
  HIRED: 'bg-emerald-500', REJECTED: 'bg-red-500', WITHDRAWN: 'bg-slate-300',
};
...
export default function RecruitmentPage() {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-[1600px] mx-auto pb-12">
      ...
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 bg-white p-6 sm:p-8 rounded-[2.5rem] border border-slate-100 shadow-sm relative overflow-hidden">
        <div className="absolute top-0 left-0 w-1.5 h-full bg-indigo-600 rounded-l-[2.5rem]" />
        <div className="pl-4">
          <h2 className="text-2xl font-black text-slate-900 tracking-tighter uppercase">Talent Acquisition & ATS</h2>
          <p className="text-[10px] font-black text-slate-400 mt-1 uppercase tracking-[0.2em]">Job Postings · Candidate Pool · Pipeline · MCF Compliance</p>
        </div>
        <div className="flex flex-wrap gap-3 pl-4 sm:pl-0">
          <button onClick={() => { setCandidateForm(EMPTY_CANDIDATE); setCandidateJobId(''); setCandidateModal(true); }}
            className="px-5 py-2.5 rounded-xl text-[10px] font-black text-slate-600 bg-slate-50 border border-slate-200 hover:bg-slate-100 uppercase tracking-widest transition-all">
            + Add Candidate
          </button>
          <button onClick={() => { setEditingJob(null); setJobForm(EMPTY_JOB); setJobModal(true); }}
            className="px-5 py-2.5 rounded-xl text-[10px] font-black text-white bg-indigo-600 hover:bg-indigo-700 shadow-lg shadow-indigo-500/20 uppercase tracking-widest transition-all">
            + New Job Opening
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Open Positions', value: openJobs, sub: `${jobs.filter(j => j.status === 'DRAFT').length} drafts`, color: 'text-indigo-600' },
          { label: 'Total Candidates', value: totalApplicants, sub: `${candidates.filter(c => c.stage === 'HIRED').length} hired · ${candidates.filter(c => !c.jobId).length} in pool`, color: 'text-emerald-600' },
          { label: 'Upcoming Interviews', value: upcomingInterviews, sub: 'scheduled ahead', color: 'text-amber-600' },
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden flex flex-col min-h-[600px]">
        <div className="px-4 sm:px-8 border-b border-slate-100 flex items-center gap-5 sm:gap-8 bg-slate-50/40 overflow-x-auto custom-scrollbar">
          {([
            ...
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              className={`py-5 text-[10px] font-black uppercase tracking-[0.18em] border-b-2 transition-all flex items-center gap-2 shrink-0 whitespace-nowrap ${activeTab === tab.key ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-400 hover:text-slate-600'}`}>
              {tab.label}
              {'count' in tab && tab.count !== undefined && (
                <span className={`text-[8px] font-black px-1.5 py-0.5 rounded-full ${activeTab === tab.key ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-100 text-slate-500'}`}>{tab.count}</span>
              )}
            </button>
          ))}
        </div>
          ...
          <div className="overflow-x-auto">
            {loading ? (
              ...
              <div className="p-20 flex flex-col items-center gap-4">
                <div className="w-16 h-16 bg-slate-50 border border-slate-200 rounded-2xl flex items-center justify-center text-2xl">📋</div>
                <p className="text-sm font-black text-slate-400 uppercase tracking-widest">No job openings yet</p>
                <button onClick={() => { setEditingJob(null); setJobForm(EMPTY_JOB); setJobModal(true); }} className="px-5 py-2.5 bg-indigo-600 text-white text-[10px] font-black rounded-xl uppercase tracking-widest">Create First Opening</button>
              </div>
            ) : (
                    ...
                    return (
                      <tr key={job.id} className="hover:bg-slate-50/60 transition-all">
                        <td className="px-8 py-5">
                          ...
                          <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-1">{job.location || 'Location TBD'} · {job.jobType.replace('_', ' ')} · {job.headcount} HC</p>
                          {/* Job lifecycle bar */}
                          <div className="flex items-center gap-1 mt-2">
                            {JOB_STATUS_FLOW.map((s, i) => (
                              <div key={s} className={`flex items-center gap-1 ${i < JOB_STATUS_FLOW.length - 1 ? 'flex-1' : ''}`}>
                                <div className={`w-2 h-2 rounded-full flex-shrink-0 ${i <= statusIdx ? (s === 'CANCELLED' ? 'bg-red-400' : 'bg-indigo-500') : 'bg-slate-200'}`} />
                                {i < JOB_STATUS_FLOW.length - 1 && <div className={`flex-1 h-px ${i < statusIdx ? 'bg-indigo-300' : 'bg-slate-200'}`} />}
                              </div>
                            ))}
                          </div>
                          <div className="flex gap-3 mt-1">
                            {JOB_STATUS_FLOW.map((s, i) => (
                              <span key={s} className={`text-[8px] font-black uppercase tracking-widest ${i === statusIdx ? 'text-indigo-600' : 'text-slate-300'}`}>{s}</span>
                            ))}
                          </div>
                        </td>
                        <td className="px-4 py-5">
                          <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest border border-slate-200 px-2.5 py-1 rounded-lg">{job.department}</span>
                        </td>
                        <td className="px-4 py-5">
                          <span className={`text-[9px] px-2.5 py-1 rounded-full font-black uppercase border tracking-widest ${JOB_STATUS_COLORS[job.status] || 'bg-slate-100 text-slate-500 border-slate-200'}`}>{job.status}</span>
                        </td>
                        <td className="px-4 py-5">
                          {job.mcfPostedAt ? (
                            <div>
                              <div className="flex items-center gap-1.5">
                                ...
                                <span className="text-[9px] font-black text-emerald-700 uppercase tracking-widest">Listed</span>
                              </div>
                              {daysLeft !== null && <p className="text-[9px] text-slate-400 font-bold mt-0.5">{daysLeft > 0 ? `${daysLeft}d window` : 'Window closed'}</p>}
                            </div>
                          ) : (
                            <button onClick={() => { setMcfJobId(job.id); setMcfForm(EMPTY_MCF); setMcfModal(true); }}
                              className="text-[9px] font-black text-orange-600 border border-orange-200 bg-orange-50 px-2 py-1 rounded-lg uppercase tracking-widest hover:bg-orange-100 transition-all">
                              Record MCF
                            </button>
                          )}
                        </td>
                        <td className="px-4 py-5 min-w-[220px]">
                          {total === 0 ? (
                            <span className="text-[9px] text-slate-300 font-bold">No candidates yet</span>
                          ) : (
                            <div>
                                ...
                                {ACTIVE_STAGES.map(s => {
                                  ...
                                  return (
                                    <div key={s} title={`${STAGE_LABELS[s]}: ${n}`} className="flex-1 flex flex-col justify-end">
                                      <div className={`w-full rounded-t-sm ${STAGE_DOT[s]} opacity-80`} style={{ height: `${Math.max(pct, n > 0 ? 15 : 0)}%` }} />
                                    </div>
                                  );
                                })}
                              </div>
                              <div className="flex gap-1 mt-1.5 flex-wrap">
                                {ACTIVE_STAGES.filter(s => counts[s]).map(s => (
                                  <span key={s} className={`text-[8px] font-black border px-1.5 py-0.5 rounded ${STAGE_COLORS[s]}`}>
                                    {counts[s]} {STAGE_LABELS[s]}
                                  </span>
                                ))}
                                {(counts['REJECTED'] || 0) > 0 && <span className="text-[8px] font-black border px-1.5 py-0.5 rounded bg-red-50 text-red-500 border-red-100">{counts['REJECTED']} Rejected</span>}
                              </div>
                            </div>
                          )}
                        </td>
                        <td className="px-8 py-5 text-right">
                          <div className="flex items-center justify-end gap-2">
                            <button onClick={() => { setCandidateForm(EMPTY_CANDIDATE); setCandidateJobId(job.id); setCandidateModal(true); }}
                              className="text-[9px] font-black text-indigo-600 border border-indigo-200 bg-indigo-50 px-2.5 py-1.5 rounded-lg uppercase tracking-widest hover:bg-indigo-100 transition-all">
                              + Candidate
                            </button>
                            <button onClick={() => { setPipelineJob(job.id); setPipelineStage(''); setActiveTab('pipeline'); }}
                              className="text-[9px] font-black text-slate-500 border border-slate-200 bg-slate-50 px-2.5 py-1.5 rounded-lg uppercase tracking-widest hover:bg-slate-100 transition-all">
                              Pipeline
                            </button>
                            ...
                            {job.status !== 'CANCELLED' && (
                              <button onClick={() => archiveJob(job.id)}
                                className="text-[9px] font-black text-red-500 border border-red-100 bg-red-50 px-2.5 py-1.5 rounded-lg uppercase tracking-widest hover:bg-red-100 transition-all">
                                Archive
                              </button>
                            )}
                          </div>
        ...
        {activeTab === 'candidates' && (
          <div className="flex flex-col">
            ...
            {loading ? (
              ...
              <div className="p-20 flex flex-col items-center gap-4">
                <div className="w-16 h-16 bg-slate-50 border border-slate-200 rounded-2xl flex items-center justify-center text-2xl">👤</div>
                <p className="text-sm font-black text-slate-400 uppercase tracking-widest">{candidateSearch ? 'No matching candidates' : 'No candidates yet'}</p>
                {!candidateSearch && <button onClick={() => { setCandidateForm(EMPTY_CANDIDATE); setCandidateJobId(''); setCandidateModal(true); }} className="px-5 py-2.5 bg-indigo-600 text-white text-[10px] font-black rounded-xl uppercase tracking-widest">Add First Candidate</button>}
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                      ...
                      <tr key={c.id} className="hover:bg-slate-50/60 transition-all cursor-pointer" onClick={() => loadCandidateDetail(c.id)}>
                        <td className="px-8 py-5">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-xl bg-indigo-50 border border-indigo-100 flex items-center justify-center text-xs font-black text-indigo-600 flex-shrink-0">
                              {c.firstName[0]}{c.lastName[0]}
                            </div>
                            <div>
                              <p className="text-sm font-black text-slate-900">{c.firstName} {c.lastName}</p>
                              <p className="text-[9px] text-slate-400 font-bold mt-0.5">{c.email}</p>
                              {c.currentTitle && <p className="text-[9px] text-slate-400 font-bold">{c.currentTitle}{c.currentEmployer ? ` @ ${c.currentEmployer}` : ''}</p>}
                            </div>
                          </div>
                        </td>
                        <td className="px-4 py-5" onClick={e => e.stopPropagation()}>
                          <select value={c.stage} onChange={e => updateStage(c.id, e.target.value)}
                            className={`text-[9px] font-black border rounded-lg px-2.5 py-1.5 uppercase tracking-widest cursor-pointer ${STAGE_COLORS[c.stage] || 'bg-slate-100 text-slate-600 border-slate-200'}`}>
                            {PIPELINE_STAGES.map(s => <option key={s} value={s}>{STAGE_LABELS[s]}</option>)}
                          </select>
                        </td>
                        <td className="px-4 py-5">
                          {c.job ? (
                            <span className="text-[9px] font-black text-slate-600 border border-slate-200 bg-slate-50 px-2 py-1 rounded-lg uppercase tracking-widest">{c.job.title}</span>
                          ) : (
                            <span className="text-[9px] font-bold text-slate-300 italic">Unassigned</span>
                          )}
                        </td>
                        <td className="px-4 py-5"><span className="text-sm font-black text-slate-600">{c.expectedSalary ? `$${Number(c.expectedSalary).toLocaleString()}` : '—'}</span></td>
                        <td className="px-4 py-5"><span className="text-[9px] font-bold text-slate-500">{c.noticePeriod ? `${c.noticePeriod}d` : '—'}</span></td>
                        <td className="px-4 py-5"><span className="text-[9px] font-bold text-slate-400">{fmtDate(c.createdAt)}</span></td>
                        <td className="px-4 py-5" onClick={e => e.stopPropagation()}>
                          {c.resumeName ? (
                            <div className="flex items-center gap-1.5">
                              <button onClick={() => downloadResume(c.id)} title={c.resumeName}
                                className="text-[9px] font-black text-emerald-700 border border-emerald-200 bg-emerald-50 px-2 py-1 rounded-lg uppercase tracking-widest hover:bg-emerald-100 transition-all max-w-[90px] truncate">
                                {c.resumeName.length > 12 ? c.resumeName.slice(0, 10) + '…' : c.resumeName}
                              </button>
                              <button onClick={() => deleteResume(c.id)} title="Remove resume"
                                className="text-[9px] font-black text-red-400 hover:text-red-600 transition-colors">✕</button>
                            </div>
                          ) : (
                            <label className={`text-[9px] font-black border rounded-lg px-2 py-1 uppercase tracking-widest transition-all cursor-pointer ${resumeUploading === c.id ? 'text-slate-400 border-slate-100 bg-slate-50' : 'text-indigo-600 border-indigo-200 bg-indigo-50 hover:bg-indigo-100'}`}>
          ...
          <div className="flex flex-col">
            <div className="px-8 py-4 border-b border-slate-100 flex items-center gap-4">
              <select value={pipelineJob} onChange={e => setPipelineJob(e.target.value)}
                className="text-[10px] font-black text-slate-600 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 uppercase tracking-widest">
            ...
            {!pipelineStage && (
              <div className="px-8 py-4 border-b border-slate-50 grid grid-cols-9 gap-2">
                {PIPELINE_STAGES.map(s => {
                  const n = filteredPipeline.filter(c => c.stage === s).length;
                  return (
                    <button key={s} onClick={() => setPipelineStage(s === pipelineStage ? '' : s)}
                      className={`flex flex-col items-center gap-1.5 p-3 rounded-xl border transition-all hover:shadow-sm ${n > 0 ? STAGE_COLORS[s] : 'bg-slate-50 text-slate-300 border-slate-100'}`}>
            ...
            {loading ? (
              ...
              <div className="p-20 flex flex-col items-center gap-4">
                <div className="w-16 h-16 bg-slate-50 border border-slate-200 rounded-2xl flex items-center justify-center text-2xl">👤</div>
                <p className="text-sm font-black text-slate-400 uppercase tracking-widest">No candidates in pipeline</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {sortedPipeline.map(c => (
                      <tr key={c.id} className="hover:bg-slate-50/60 transition-all cursor-pointer" onClick={() => loadCandidateDetail(c.id)}>
                        ...
                        <td className="px-4 py-5">
                          <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest border border-slate-200 px-2 py-1 rounded-lg">
                            {c.job?.title || jobs.find(j => j.id === c.jobId)?.title || <span className="text-slate-300 normal-case font-bold not-italic">Unassigned</span>}
                          </span>
                        </td>
                        <td className="px-4 py-5" onClick={e => e.stopPropagation()}>
                          <select value={c.stage} onChange={e => updateStage(c.id, e.target.value)}
                            className={`text-[9px] font-black border rounded-lg px-2.5 py-1.5 uppercase tracking-widest ${STAGE_COLORS[c.stage] || 'bg-slate-100 text-slate-600 border-slate-200'}`}>
                            {PIPELINE_STAGES.map(s => <option key={s} value={s}>{STAGE_LABELS[s]}</option>)}
                          </select>
                        </td>
                        <td className="px-4 py-5"><span className="text-sm font-black text-slate-600">{c.expectedSalary ? `$${Number(c.expectedSalary).toLocaleString()}` : '—'}</span></td>
        ...
        {activeTab === 'interviews' && (
          <div className="overflow-x-auto">
            {loading ? (
              ...
              <div className="p-20 flex flex-col items-center gap-4">
                <div className="w-16 h-16 bg-slate-50 border border-slate-200 rounded-2xl flex items-center justify-center text-2xl">📅</div>
                <p className="text-sm font-black text-slate-400 uppercase tracking-widest">No interviews scheduled</p>
              </div>
            ) : (
              <table className="w-full text-left">
                <thead>
                ...
                <tbody className="divide-y divide-slate-50">
                  {sortedInterviews.map(ir => {
                    ...
                    return (
                      <tr key={ir.id} className="hover:bg-slate-50/60 transition-all">
                        <td className="px-8 py-5"><p className="text-sm font-black text-slate-900">{ir.roundName}</p></td>
                        <td className="px-4 py-5">
                          {cand ? (
                            <button onClick={() => loadCandidateDetail(cand.id)} className="text-left hover:opacity-80">
                              <p className="text-sm font-black text-slate-800">{cand.firstName} {cand.lastName}</p>
                              <p className="text-[9px] text-slate-400 font-bold">{cand.job?.title || jobs.find(j => j.id === cand.jobId)?.title || '—'}</p>
                            </button>
                          ) : <span className="text-slate-400 text-xs">—</span>}
                        </td>
                        <td className="px-4 py-5">
                          <p className={`text-sm font-black ${isPast ? 'text-slate-400' : 'text-slate-900'}`}>{fmtDateTime(ir.scheduledAt)}</p>
                          {!isPast && <span className="text-[9px] font-black text-emerald-600 uppercase tracking-widest">Upcoming</span>}
                        </td>
                        <td className="px-4 py-5"><p className="text-xs text-slate-500 max-w-[240px] truncate">{ir.notes || '—'}</p></td>
                        <td className="px-4 py-5">
                          {ir.result ? (
                            <span className={`text-[9px] font-black uppercase border px-2.5 py-1 rounded-full tracking-widest ${ir.result === 'PASS' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : ir.result === 'FAIL' ? 'bg-red-50 text-red-600 border-red-100' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>{ir.result}</span>
      ...
      {detailCandidate && (
        <div className="fixed inset-0 z-50 flex justify-end" onClick={() => setDetailCandidate(null)}>
          ...
          <div className="relative z-10 w-full max-w-lg bg-white h-full overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
            {detailLoading ? (
              <div className="p-16 text-center text-slate-400 text-sm font-bold">Loading…</div>
            ) : (
              <>
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="flex items-center gap-3 mb-1">
                        ...
                        <div>
                          <h3 className="text-lg font-black text-slate-900 tracking-tighter">{detailCandidate.firstName} {detailCandidate.lastName}</h3>
                          <p className="text-xs font-bold text-slate-500">{detailCandidate.email}</p>
                        </div>
                      </div>
                      {detailCandidate.phone && <p className="text-xs font-bold text-slate-400 mt-1">{detailCandidate.phone}</p>}
                    </div>
                    <button onClick={() => setDetailCandidate(null)} className="text-slate-400 hover:text-slate-600 text-xl font-black">✕</button>
                  </div>
                  <div className="mt-4 flex flex-wrap gap-2">
                    <span className={`text-[9px] font-black border rounded-full px-2.5 py-1 uppercase tracking-widest ${STAGE_COLORS[detailCandidate.stage] || ''}`}>
                      {STAGE_LABELS[detailCandidate.stage] || detailCandidate.stage}
                    </span>
                    {detailCandidate.job && (
                      <span className="text-[9px] font-black bg-slate-50 text-slate-600 border border-slate-200 rounded-full px-2.5 py-1 uppercase tracking-widest">{detailCandidate.job.title}</span>
                    )}
                    {detailCandidate.isHired && <span className="text-[9px] font-black bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-full px-2.5 py-1 uppercase tracking-widest">Hired</span>}
                    {detailCandidate.isOfferMade && !detailCandidate.isHired && <span className="text-[9px] font-black bg-orange-50 text-orange-700 border border-orange-100 rounded-full px-2.5 py-1 uppercase tracking-widest">Offer Made</span>}
                  </div>
                </div>
                ...
                <div className="p-8 flex flex-col gap-7">
                        ...
                        {PIPELINE_STAGES.filter(s => s !== 'WITHDRAWN').map((s) => {
                          ...
                          return (
                            <div key={s} className="flex items-start gap-4 py-2 relative z-10">
                              <div className={`w-[18px] h-[18px] rounded-full flex-shrink-0 flex items-center justify-center mt-0.5 border-2 ${
                                ...
                                {(isPast) && <div className="w-2 h-2 bg-white rounded-full" />}
                                {isCurrent && <div className="w-2 h-2 bg-white rounded-full" />}
                              </div>
                              <div className="flex-1 min-w-0 pb-1">
                                <div className="flex items-center justify-between">
                                  <span className={`text-[10px] font-black uppercase tracking-widest ${isCurrent || isRejected ? 'text-slate-900' : isPast ? 'text-slate-400' : 'text-slate-300'}`}>
                                    {STAGE_LABELS[s]}
                                  </span>
                                  {event && <span className="text-[8px] font-bold text-slate-400">{fmtDate(event.createdAt)}</span>}
                                  {isCurrent && !event && <span className="text-[8px] font-black text-indigo-500 uppercase tracking-widest">Current</span>}
                                </div>
                                {event?.note && <p className="text-[9px] text-slate-400 mt-0.5">{event.note}</p>}
                              </div>
                            </div>
                          );
                        })}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {PIPELINE_STAGES.map(s => (
                        <button key={s} onClick={() => updateStage(detailCandidate.id, s)}
                          className={`text-[9px] font-black border rounded-lg px-2.5 py-1.5 uppercase tracking-widest transition-all ${detailCandidate.stage === s ? 'ring-2 ring-indigo-400 ' + STAGE_COLORS[s] : STAGE_COLORS[s] + ' opacity-60 hover:opacity-100'}`}>
                          {STAGE_LABELS[s]}
                        </button>
                      ))}
                    </div>
                  ...
                  <div>
                    ...
                    {detailCandidate.job ? (
                      <div className="bg-slate-50 border border-slate-100 rounded-xl p-3">
                        <p className="text-xs font-black text-slate-800">{detailCandidate.job.title}</p>
                        {detailCandidate.job.department && <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">{detailCandidate.job.department}</p>}
                      </div>
                    ) : <p className="text-xs text-slate-400 font-bold">Not tagged to any job opening</p>}
                  </div>
                  ...
                  <div>
                    ...
                    {detailCandidate.resumeName ? (
                      <button onClick={() => downloadResume(detailCandidate.id)}
                        className="w-full flex items-center gap-3 bg-slate-50 hover:bg-emerald-50 border border-slate-200 hover:border-emerald-200 rounded-xl p-3 transition-all group">
                        <div className="w-8 h-8 rounded-lg bg-red-50 border border-red-100 flex items-center justify-center text-xs font-black text-red-500 flex-shrink-0">PDF</div>
                        <div className="flex-1 min-w-0 text-left">
                          <p className="text-xs font-black text-slate-800 truncate group-hover:text-emerald-700">{detailCandidate.resumeName}</p>
                          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Click to download</p>
                        </div>
                      </button>
                    ) : (
                      <label className={`w-full flex items-center justify-center gap-2 border-2 border-dashed rounded-xl p-4 cursor-pointer transition-all ${resumeUploading === detailCandidate.id ? 'border-slate-200 bg-slate-50' : 'border-indigo-200 hover:border-indigo-400 hover:bg-indigo-50'}`}>
                        <div className="text-center">
                          <p className={`text-[10px] font-black uppercase tracking-widest ${resumeUploading === detailCandidate.id ? 'text-slate-400' : 'text-indigo-600'}`}>
                            {resumeUploading === detailCandidate.id ? 'Uploading…' : 'Upload Resume'}
                          </p>
                          <p className="text-[8px] font-bold text-slate-400 mt-1">PDF, DOC, DOCX · Max 10 MB</p>
                        </div>
                  ...
                  <div>
                    ...
                    {!detailCandidate.interviewRounds?.length ? (
                      <p className="text-xs text-slate-400 font-bold">No interviews scheduled yet.</p>
                    ) : (
                      <div className="flex flex-col gap-3">
                        {detailCandidate.interviewRounds.map(ir => (
                          <div key={ir.id} className="bg-slate-50 border border-slate-100 rounded-xl p-4">
                            <div className="flex justify-between">
                              <p className="text-xs font-black text-slate-900">{ir.roundName}</p>
                              {ir.result && (
                                <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-full ${ir.result === 'PASS' ? 'bg-emerald-50 text-emerald-700' : ir.result === 'FAIL' ? 'bg-red-50 text-red-600' : 'bg-slate-100 text-slate-500'}`}>{ir.result}</span>
                              )}
                            </div>
                            <p className="text-[9px] font-bold text-slate-500 mt-1">{fmtDateTime(ir.scheduledAt)}</p>
                            {ir.notes && <p className="text-[10px] text-slate-500 mt-1">{ir.notes}</p>}
                          </div>
                        ))}
                      </div>
                    )}
                      ...
                      <div className="flex flex-col gap-2">
                        {[...detailCandidate.stageEvents].reverse().map(ev => (
                          <div key={ev.id} className="flex items-start gap-3">
                            <div className={`w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0 ${STAGE_DOT[ev.toStage] || 'bg-slate-300'}`} />
                            <div>
                              ...
                              {ev.note && <p className="text-[9px] text-slate-400">{ev.note}</p>}
                              <p className="text-[8px] font-bold text-slate-300 mt-0.5">{fmtDateTime(ev.createdAt)}</p>
                            </div>
                          </div>
                        ))}
                      </div>
      ...
      {tagJobModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setTagJobModal(null)}>
          ...
          <div className="relative z-10 bg-white rounded-[2rem] shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
            <div className="p-8 border-b border-slate-100 flex justify-between items-center">
              <div>
                <h3 className="text-base font-black text-slate-900 tracking-tighter">Tag to Job Opening</h3>
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">Select a job to link this candidate</p>
              </div>
              <button onClick={() => setTagJobModal(null)} className="text-slate-400 hover:text-slate-600 text-xl font-black">✕</button>
            </div>
            <div className="p-8 flex flex-col gap-4">
              ...
              <p className="text-[9px] text-slate-400 font-bold">Selecting no job moves the candidate back to the unassigned pool.</p>
            </div>
            <div className="p-8 pt-0 flex gap-3 justify-end">
              <button onClick={() => setTagJobModal(null)} className="px-6 py-3 text-[10px] font-black text-slate-600 bg-slate-50 border border-slate-200 rounded-xl uppercase tracking-widest">Cancel</button>
              <button onClick={tagJob} disabled={tagJobSaving}
                className="px-6 py-3 text-[10px] font-black text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl uppercase tracking-widest disabled:opacity-50">
      ...
      {jobModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => { setJobModal(false); setEditingJob(null); }}>
          <div className="absolute inset-0 bg-black/30 backdrop-blur-sm" />
          <div className="relative z-10 bg-white rounded-[2rem] shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="p-8 border-b border-slate-100 flex justify-between items-center">
              <h3 className="text-lg font-black text-slate-900 tracking-tighter">{editingJob ? 'Edit Job Opening' : 'New Job Opening'}</h3>
              <button onClick={() => { setJobModal(false); setEditingJob(null); }} className="text-slate-400 hover:text-slate-600 text-xl font-black">✕</button>
            </div>
            <div className="p-8 flex flex-col gap-5">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Headcount</label>
                  <input type="number" min={1} value={String(jobForm.headcount || 1)} onChange={e => setJobForm(p => ({ ...p, headcount: e.target.value }))}
                    className="border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-400" />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Job Type</label>
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Salary Min (SGD)</label>
                  <input type="number" value={String(jobForm.salaryMin || '')} onChange={e => setJobForm(p => ({ ...p, salaryMin: e.target.value }))} placeholder="e.g. 4000"
                    className="border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-400" />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Salary Max (SGD)</label>
                ...
                <div className="col-span-2 flex flex-col gap-1.5">
                  <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Requirements</label>
            ...
            <div className="p-8 pt-0 flex gap-3 justify-end">
              <button onClick={() => { setJobModal(false); setEditingJob(null); }} className="px-6 py-3 text-[10px] font-black text-slate-600 bg-slate-50 border border-slate-200 rounded-xl uppercase tracking-widest hover:bg-slate-100">Cancel</button>
              <button onClick={saveJob} disabled={jobSaving || !jobForm.title || !jobForm.department || !jobForm.jobDescription}
                className="px-6 py-3 text-[10px] font-black text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl uppercase tracking-widest disabled:opacity-50 transition-all">
      ...
      {mcfModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setMcfModal(false)}>
          ...
          <div className="relative z-10 bg-white rounded-[2rem] shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
            <div className="p-8 border-b border-slate-100 flex justify-between items-center">
              <div>
                <h3 className="text-base font-black text-slate-900 tracking-tighter">Record MCF Listing</h3>
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">MyCareersFuture 14-Day Compliance</p>
              </div>
              <button onClick={() => setMcfModal(false)} className="text-slate-400 hover:text-slate-600 text-xl font-black">✕</button>
            </div>
            <div className="p-8 flex flex-col gap-4">
              <p className="text-xs text-slate-500 leading-relaxed">Under the Fair Consideration Framework, job postings must be listed on MyCareersFuture for at least 14 days before shortlisting.</p>
              <div className="flex flex-col gap-1.5">
                <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest">MCF Job ID (Reference)</label>
                <input value={mcfForm.mcfJobId} onChange={e => setMcfForm(p => ({ ...p, mcfJobId: e.target.value }))} placeholder="MCF-2026-XXXXXX"
                  className="border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-400" />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Date Posted on MCF *</label>
                <input type="date" value={mcfForm.mcfPostedAt} onChange={e => setMcfForm(p => ({ ...p, mcfPostedAt: e.target.value }))}
                  className="border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-400" />
              </div>
            </div>
            <div className="p-8 pt-0 flex gap-3 justify-end">
              <button onClick={() => setMcfModal(false)} className="px-6 py-3 text-[10px] font-black text-slate-600 bg-slate-50 border border-slate-200 rounded-xl uppercase tracking-widest">Cancel</button>
      ...
      {candidateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setCandidateModal(false)}>
          <div className="absolute inset-0 bg-black/30 backdrop-blur-sm" />
          <div className="relative z-10 bg-white rounded-[2rem] shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="p-8 border-b border-slate-100 flex justify-between items-center">
              <h3 className="text-lg font-black text-slate-900 tracking-tighter">Add Candidate</h3>
              <button onClick={() => setCandidateModal(false)} className="text-slate-400 hover:text-slate-600 text-xl font-black">✕</button>
            </div>
            <div className="p-8 flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Job Opening <span className="text-slate-300 normal-case font-bold">(optional — can be tagged later)</span></label>
            ...
            <div className="p-8 pt-0 flex gap-3 justify-end">
              <button onClick={() => setCandidateModal(false)} className="px-6 py-3 text-[10px] font-black text-slate-600 bg-slate-50 border border-slate-200 rounded-xl uppercase tracking-widest">Cancel</button>
      ...
      {interviewModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setInterviewModal(false)}>
          ...
          <div className="relative z-10 bg-white rounded-[2rem] shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
            <div className="p-8 border-b border-slate-100 flex justify-between items-center">
              <h3 className="text-lg font-black text-slate-900 tracking-tighter">Schedule Interview</h3>
              <button onClick={() => setInterviewModal(false)} className="text-slate-400 hover:text-slate-600 text-xl font-black">✕</button>
            </div>
            <div className="p-8 flex flex-col gap-4">
              {(() => { const c = candidates.find(x => x.id === interviewCandidateId); return c ? <p className="text-xs font-bold text-slate-500">Candidate: <span className="text-slate-800">{c.firstName} {c.lastName}</span></p> : null; })()}
              <div className="flex flex-col gap-1.5">
                <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Round Name *</label>
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Scheduled Date & Time *</label>
            ...
            <div className="p-8 pt-0 flex gap-3 justify-end">
              <button onClick={() => setInterviewModal(false)} className="px-6 py-3 text-[10px] font-black text-slate-600 bg-slate-50 border border-slate-200 rounded-xl uppercase tracking-widest">Cancel</button>
